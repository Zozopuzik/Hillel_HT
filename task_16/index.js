...
let createUserHamburger = (formData) =>{
  formData.burgerSize === 'small' ? formData.burgerSize = Hamburger.SIZE_SMALL : formData.burgerSize = Hamburger.SIZE_BIG
...
let createOrder = (obj) =>{
  let orderMessageInput = document.querySelector('textarea[name="orderMessage"]');
...
Ваш заказ:
<br/>
<b>${obj.size.size}</b> бургер с <b>${obj.stuffing.stuffing}</b> и <b>${obj.topping ? toppingArr.join(', '): 'без добавок' }</b> будет готов в течении <b>${Math.floor(Math.random() * 10) + 1}</b> минут
</br>
